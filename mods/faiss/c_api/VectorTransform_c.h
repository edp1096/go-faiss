 ...
 */
FAISS_C_API int faiss_VectorTransform_train(
 ...
 */
FAISS_C_API void faiss_VectorTransform_apply_noalloc(
        const FaissVectorTransform* vt,
        idx_t n,
        const float* x,
        float* xt);
...
/// approximate result
FAISS_C_API void faiss_VectorTransform_reverse_transform(
        const FaissVectorTransform* vt,
        idx_t n,
        const float* xt,
        float* x);
...
FAISS_C_API void faiss_LinearTransform_transform_transpose(
...
FAISS_C_API void faiss_LinearTransform_set_is_orthonormal(FaissLinearTransform* vt);
...
FAISS_C_API int faiss_RandomRotationMatrix_new_with(
...
FAISS_C_API int faiss_PCAMatrix_new_with(
        FaissPCAMatrix** p_vt,
        int d_in,
        int d_out,
        float eigen_power,
...
FAISS_C_API int faiss_ITQMatrix_new_with(FaissITQMatrix** p_vt, int d);

FAISS_DECLARE_CLASS_INHERITED(ITQTransform, VectorTransform)
FAISS_DECLARE_DESTRUCTOR(ITQTransform)

FAISS_C_API int faiss_ITQTransform_new_with(
...
FAISS_C_API int faiss_OPQMatrix_new_with(FaissOPQMatrix** p_vt, int d, int M, int d2);
...
FAISS_C_API int faiss_RemapDimensionsTransform_new_with(
...
FAISS_C_API int faiss_NormalizationTransform_new_with(
...
FAISS_C_API int faiss_CenteringTransform_new_with(FaissCenteringTransform** p_vt, int d);
